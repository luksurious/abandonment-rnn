...
def train_model(x, y, prefix, patience=3, folds=10, undersample=False, units=50, augment_train=True):
    print("\n---------------------------------\n")
    print("Got %d data points. 0: %d, 1: %d" % (len(x), len(y) - np.count_nonzero(y), np.count_nonzero(y)))

    x_resampled, y_resampled = presplit_resampling(augment_train, undersample, x, y)
    ...
    if folds > 1:
        ...
        for index, (train_index, val_index) in enumerate(kf.split(x_train, y_train)):
            ...
            x_train_fold, y_train_fold = postsplit_resampling(augment_train, undersample, x_test, x_train_fold,
                                                              y_train_fold)
            ...
            history = run_train(x_train_fold, y_train_fold, x_val, y_val, x_val, y_val, folded_scores,
        ...
        history = run_train(x_train_fold, y_train_fold, x_val, y_val, x_test, y_test,
...
def postsplit_resampling(augment_train, undersample, x_test, x_train, y_train):
    if augment_train:
...
def presplit_resampling(augment_train, undersample, x, y):
...
def run_train(x_train, y_train, x_val, y_val, x_test, y_test, folded_scores, index, prefix, patience, units):
    ...
    # Train the model.
    model = create_model(units, x_train[0].shape)
    # print(model.summary())
    history = model.fit(
        x_train,
        y_train,
