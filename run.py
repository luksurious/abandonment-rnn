...
def setup_arguments():
    ...
    parser.add_argument('--repeated_eval', action='store_true', help='perform repeated evaluation to average '
...
if __name__ == '__main__':
    ...
    if args.optuna is True:
        ...
        def optuna_objective(trial):
            ...
            # args.batch_size = trial.suggest_categorical('batch_size', [8, 16])
            ...
            # Current best value is 0.7532838338132455 with parameters:
            # {'lr': 0.0002875771633554991,
            # 'optimizer': 'RMSprop',
            # 'batch_size': 2,
            # 'dropout': 0.1,
            # 'dropout_only_last': False,
            # 'attention_first': True,
            # 'attention_middle': False}.

            # 0.7796900254589042 with parameters: {
            # 'attention_first': False,
            # 'batch_size': 4,
            # 'dropout': 0.2,
            # 'dropout_only_last': False,
            # 'lr': 0.0031475321427673762,
            # 'optimizer': 'RMSprop'}

            return np.mean(vals[:, 3])

        study = optuna.create_study(direction='maximize', study_name='mouse_model',
                                    storage='sqlite:///mouse_model.db', load_if_exists=True)
